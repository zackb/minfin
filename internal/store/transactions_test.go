...
func TestTransactionsAccountFilter(t *testing.T) {
	s := seedStore(t)
	start, end := wideRange()
	rows, _, err := s.Transactions(TxnFilter{Start: start, End: end, AccountID: "a1", Direction: "all"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestTransactionsDirectionFilter(t *testing.T) {
	s := seedStore(t)
	start, end := wideRange()
	debits, _, _ := s.Transactions(TxnFilter{Start: start, End: end, Direction: "debit"})
	...
	credits, _, _ := s.Transactions(TxnFilter{Start: start, End: end, Direction: "credit"})
...
func TestTransactionsSearch(t *testing.T) {
	s := seedStore(t)
	start, end := wideRange()
	rows, _, _ := s.Transactions(TxnFilter{Start: start, End: end, Direction: "all", Query: "Coffee"})
...
func TestTransactionsPagination(t *testing.T) {
	...
	p1, hasNext, _ := s.Transactions(TxnFilter{Start: start, End: end, Direction: "debit", Limit: 2})
	if len(p1) != 2 || !hasNext {
		t.Fatalf("page1: %d rows, hasNext=%v; want 2,true", len(p1), hasNext)
	}
	p2, hasNext, _ := s.Transactions(TxnFilter{Start: start, End: end, Direction: "debit", Limit: 2, Offset: 2})
	if len(p2) != 2 || hasNext {
